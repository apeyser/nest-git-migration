...
echo >  "${TEST_LOGFILE}" "NEST v. 2.6.0 testsuite log"
echo >> "${TEST_LOGFILE}" "======================"
...
if test "x${TEST_PYNEST}" = xtrue ; then
	...
	echo "------------------------------"
        ...
        nosetests -v --with-xunit --xunit-file=${TEST_OUTDIR}/pynest_tests.xml ${exec_prefix}/lib/python2.7/site-packages/nest/tests 2>&1 \
